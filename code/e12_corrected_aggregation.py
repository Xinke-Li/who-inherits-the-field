...
Fixes three evaluation-protocol deviations documented in LEAKAGE_AUDIT_e12.md:
  ...
  F3  the Colab aggregation compared against max(M2, M3), dropping M5_gbdt_nfa,
      ...
      comparison is kept as SECONDARY for continuity with the superseded protocol.
...
Three ceilings are reported per discipline:
  ...
  maxM2M3_naive         per-seed max(M2, M3) (the superseded Colab ceiling; secondary)
...
as the superseded e12 aggregation).
...
The superseded Colab aggregation (e12_hgt_vs_baselines.json, produced by the same run)
...
def naive_consistency_check(res_dir, per_seed_graph, e1):
    ...
    with the superseded json shipped in the same run's zip. Aborts on mismatch."""
