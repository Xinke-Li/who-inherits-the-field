#!/usr/bin/env python3
"""Superseded versus corrected dumbbell plot for the seven superseded crossings of the
graph arm, serif type with no top or right spines and a faint vertical grid.
Colors follow the prior submission's rolling-origin figure: the superseded protocol
...
One row per discipline-architecture pair, sorted by the superseded gap: the open
gold point is the superseded gap to the seed-wise best of M2 and M3 with its
...
results/results_<field>/e12_hgt_vs_baselines.json (superseded) and
...
# the seven superseded crossings of Table 3 (all other pairs cross under no protocol)
...
GOLD = "#E1A100"       # superseded protocol (prior submission's GBDT/M5 line color)
...
def build():
    ...
    for i, r in enumerate(rows):
        y = n - 1 - i
        # light gray connector joining the superseded and corrected points
        ax.plot([r["naive"], r["corr"]], [y + 0.16, y - 0.16], lw=0.8,
                color=CONNECT, zorder=2)
        # superseded: thin gold interval, open marker, upper lane of the row
    ...
    handles = [
        mpl.lines.Line2D([], [], marker="o", ms=3.6, mfc="white", mec=GOLD,
                         ...
                         label="uncorrected protocol (gold)"),
