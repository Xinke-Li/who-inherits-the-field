...
those artifacts, and the paper reports the resulting verdicts for the two
transformer configurations. For the relational and attention models the paper's
reported verdicts come instead from the post hoc symmetric grid
(`code/r17_full_symmetric_grid.py`,
`results/robustness/full_symmetric_grid/`), which re-tunes RGCN and GAT under the
transformer's own grid and unified budget in all five disciplines; the chemistry
crossing quoted in the paper comes from the symmetric re-tuning in
`results/robustness/rgcn_symmetric/` and Appendix G, not from this notebook's
asymmetric-budget run. Rerunning this notebook is needed only to regenerate the
per-seed artifacts themselves.
