...
REPRODUCIBILITY: run with PYTHONHASHSEED=5. Louvain's community COUNT on the two
small graphs (econ, math) depends on set-iteration order and therefore on the
hash seed; Q is stable to two decimals under every seed probed (0 to 15), and
the large graphs (neuro 202, physics 161, chemistry 242) are seed-invariant.
The pinned seed reproduces math k=73, the value the released figure carries.
"""
...
CACHE = os.path.join(FIGDIR, "panel_cache.pkl")   # layouts and partitions, gitignored
...
def check_provenance(prov):
    """Refuse to write anything if Louvain has not reproduced the released Q and
    community count. The panel titles print both, so a drift here would change a
    number in the paper, which no layout change is allowed to do."""
    path = os.path.join(DATA, "network_modularity.json")
    if not os.path.exists(path):
        return
    old = json.load(open(path))
    bad = []
    for f, rec in prov.items():
        if f not in old:
            continue
        for key in ("louvain_Q", "n_communities", "largest_cc_nodes"):
            if old[f][key] != rec[key]:
                bad.append(f"{f}.{key}: released {old[f][key]}, recomputed {rec[key]}")
    if bad:
        print("[guard] provenance drift, nothing written:")
        for b in bad:
            print("   ", b)
        sys.exit(3)
    print("[guard] Q, community count and component size reproduce in all five")


def build_panels(skip_full):
    prov, panel_data = {}, []
    ...
    return prov, panel_data


def composite(panel_data, layout, out_base):
    """Paste-up only. Same data, same layouts, same palette in every variant.

    tworow: three panels above, two below and centered, each panel about three
    times the area of a one-row cell, so community color and hub structure stay
    readable at print size.
    onerow: five panels across a figure*, about a fifth of the text width each.
    """
    ...
    if layout == "tworow":
        ...
        scale, ewidth, nbase, nmin, ncap = 0.9, 0.8, 5, 6, 26
        title_font, width, height, top = 16, 1750, 1150, 55
    else:
        comp = make_subplots(rows=1, cols=5, subplot_titles=titles,
                             horizontal_spacing=0.012)
        cells = [(1, i) for i in range(1, 6)]
        # a fifth of the width carries a fifth of the ink: markers and edges are
        # scaled down with the cell so the panels do not fill in solid.
        scale, ewidth, nbase, nmin, ncap = 0.55, 0.5, 3, 3.5, 15
        title_font, width, height, top = 25, 1750, 430, 78
    for (r, c), (nm, G_viz, pos, part, name_map, Q, k) in zip(cells, panel_data):
        edge, node = edge_node_traces(G_viz, pos, part, name_map,
                                      scale, ewidth, False, nbase, nmin, ncap)
    ...
    comp.update_annotations(font=dict(size=title_font, family="Arial"))
    comp.update_layout(showlegend=False, plot_bgcolor="white", paper_bgcolor="white",
                       width=width, height=height, margin=dict(b=10, l=5, r=5, t=top))
    try:
        comp.write_image(out_base + ".pdf", format="pdf")
        print(f"wrote {out_base}.pdf (vector)")
    except Exception as e:
        print(f"composite PDF failed ({e}); high-DPI PNG only")
    comp.write_image(out_base + ".png", scale=2)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--layout", choices=["tworow", "onerow"], default="tworow")
    ap.add_argument("--skip-full", action="store_true",
                    help="do not regenerate the five supplement panels")
    ap.add_argument("--cache", action="store_true",
                    help="reuse panel_cache.pkl when it exists, and write it otherwise")
    ap.add_argument("--out", default=None, help="output basename under paper/figures")
    a = ap.parse_args()

    if a.cache and os.path.exists(CACHE):
        prov, panel_data = pickle.load(open(CACHE, "rb"))
        print(f"[cache] {CACHE}")
    else:
        prov, panel_data = build_panels(a.skip_full)
        if a.cache:
            pickle.dump((prov, panel_data), open(CACHE, "wb"))
    check_provenance(prov)

    base = os.path.join(FIGDIR, a.out or "F12_five_discipline_networks")
    composite(panel_data, a.layout, base)
