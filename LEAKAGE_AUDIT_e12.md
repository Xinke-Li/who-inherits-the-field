...
| F7, F8 | the e12 manifest declares three edge types where the graph has five; per-seed graph outputs are absent from `e12_results.zip`, and `e2_hgt.py:248` bypasses the loader's SHA-256 pin | documentation / reproducibility | minor |
...
## Addendum, 2026-07-24: post-audit resolution

This addendum records what happened to two defects after the audit above was
written. Nothing in the audit text or its verdict table is altered: the
construction remains CLEAN and the physics crossing remains an artifact of the
comparison, exactly as reported.

F9, the budget asymmetry, is RESOLVED for the comparison it affected.
`h_extra_gnns.py` trained the relational and attention models at 300 epochs with
patience 30 and no weight decay while `e2_hgt.py` used 200 at patience 15 with
weight decay 1e-4. The post hoc symmetric re-tuning of Appendix G gives the
chemistry relational network the transformer's own 16-configuration grid and
unified budget; the crossing survives and strengthens, +0.035 over M5' with a
student-level interval of +0.020 to +0.049 and corrected p 0.0078
(`results/robustness/rgcn_symmetric_verdict.json`). The same symmetric protocol
was then extended to RGCN and GAT in all five disciplines, ten cells
(`results/robustness/full_symmetric_grid/`); all nine new cells are null and
chemistry RGCN remains the only crossing.

F14, asymmetric validation access, is ADDRESSED by the validation-symmetric
ceiling M5', which the corrected aggregation already uses: M5' selects its
boosting iteration count on the temporal validation cohort, the same quantity the
graph models optimise at every epoch. The paper reports both M5 and M5' and
judges the three gates against M5'.
