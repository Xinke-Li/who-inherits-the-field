...
# ---------- W5, second instance: which TF-IDF footing the floors are scored on ----------
# build_features defaults to the REPAIRED path, because T2.3 re-measured M4 both
# ways and Table 2 reports the repaired values with the legacy five retained as
# superseded. student_tfidf defaults to the LEGACY path, and the asymmetry is
# deliberate: its repaired values were never re-measured, so all 70 recorded
# floor cells sit on the all-rows footing. Defaulting to legacy is what makes the
# released code reproduce the released numbers. Set STUDENT_TFIDF=repaired to
# score the leakage-free path instead.
#
# Note which way the bias runs. Fitting the vocabulary on every row lets the
# inverse-document weights see validation and test text, which inflates the
# student-only floor and therefore UNDERSTATES M1 minus floor. The pinned
# footing is conservative for the advisor-adds reading rather than favourable to
# it: mathematics gives +0.0089 on this footing and +0.0276 on the repaired one.
LEGACY_STUDENT_TFIDF = os.environ.get("STUDENT_TFIDF", "legacy") != "repaired"


def student_tfidf(df, max_features=2000, legacy=False):
    """build_features' vec_st branch only - no advisor vocabulary.

    Carries the same audit finding as build_features (W5): fitting on every row
    estimates the IDF weights and the max_features cut from validation and test
    rows. Fit on the training split and transform the rest; legacy=True keeps the
    frozen all-rows path as the audit trail. Callers pass
    LEGACY_STUDENT_TFIDF rather than a literal, so one environment variable
    moves every floor in the suite together.
    """
    ...
    if legacy or "split" not in df.columns:
        return vec.fit_transform(docs)
    vec.fit(docs[(df.split == "train").values])
