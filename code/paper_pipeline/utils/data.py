...
def build_features(df: pd.DataFrame, concepts: str = "none",
    ...
    if concepts in ("tfidf", "tfidf_legacy"):
        ...
        # Audit finding W5. The vocabulary does come from pre-window text only, but
        # fitting on every row also estimates the IDF weights and the max_features
        # cut from validation and test rows, which the tabular rungs never see.
        # 'tfidf' now fits on the training split and transforms the rest; tokens
        # absent from the training vocabulary are dropped by transform.
        # 'tfidf_legacy' preserves the frozen all-rows path as the audit trail.
        if concepts == "tfidf_legacy" or "split" not in df.columns:
            A = vec_st.fit_transform(docs_st)
            B = vec_ad.fit_transform(docs_ad)
        else:
            tr = (df.split == "train").values
            vec_st.fit(docs_st[tr])
            vec_ad.fit(docs_ad[tr])
            A = vec_st.transform(docs_st)
            B = vec_ad.transform(docs_ad)
